/*
  ...
  |giorno_corrente   |     Intero    |     Lavoro      |      Giorno della data corrente.                                                                                                            |
  |mese_corrente     |     Intero    |     Lavoro      |      Mese della data corrente.                                                                                                              |
  |anno_corrente     |     Intero    |     Lavoro      |      Anno della data corrente.                                                                                                              |
  |ora_secondi       |     Tempo     |     Lavoro      |      Secondi trascorsi dal 01/01/1970.                                                                                                      |
  |*ora_t            |     Tempo     |     Lavoro      |      Puntatore a struct tm.
  ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
*/
...
void stampa_calendario(int panno_scelto, int pprimo_giorno) {
    ...
    // Dichiarazione variabili
    int mese, vuoto, giorno, flag_bisestile, giorno_corrente, mese_corrente, anno_corrente;
	time_t ora_secondi = time(NULL);
    struct tm *ora_t = localtime(&ora_secondi);

    // Verificare che l'anno scelto sia bisestile o no
    flag_bisestile = controllo_bisestile(panno_scelto);
    
    // Giorno corrente
    giorno_corrente = ora_t->tm_mday;  
    
    // Mese corrente
    mese_corrente = ora_t->tm_mon+1;

    // Anno corrente
    anno_corrente = ora_t->tm_year+1900;
    ...
    for (mese = 0; mese < 12; mese++) {
        ...
        // Verificare che pprimo_giorno abbia superato il valore 7
        ...
        for (giorno = 1; giorno <= MAX_GIORNI_MESI[mese]; giorno++) {
            ...
            // Incrementare di 1 pprimo_giorno
            pprimo_giorno++;

            // Se il giorno e' costituito da una cifra...
            if (giorno < 10) {

                // Stampare il giorno ed uno spazio in piu' (con * se coincide con la data corrente)
                if (giorno == giorno_corrente && mese == mese_corrente-1 && panno_scelto == anno_corrente)
                    printf("%d*   ", giorno);
                else
                    printf("%d    ", giorno);
            ...
            // Se cosi' non e'...
                ...
                if (giorno == 29 && mese == 1) {
                    ...
                    // Stampare il suddetto giorno solo se l'anno e' bisestile (con * se coincide con la data corrente)
                    if (flag_bisestile) {
                        if (giorno == giorno_corrente && mese == mese_corrente-1 && panno_scelto == anno_corrente)
                            printf("\b%d*", giorno);
                        else
                    ...
                    // In caso contrario...
                        ...
                        // Decrementare di 1 pprimo_giorno
                        pprimo_giorno--;

                        // Verificare che pprimo_giorno sia sceso sotto il valore di 1
                ...
                // Altrimenti...
                    ...
                    // Stampa di un giorno del mese (con * se coincide con la data corrente)
                    if (giorno == giorno_corrente && mese == mese_corrente-1 && panno_scelto == anno_corrente)
                        printf("%d*  ", giorno);
                    else
                        printf("%d   ", giorno);
